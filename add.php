...
<body>
	...
	<script>
        ...
        render: {
            option: function(item, escape) {
                return '<div style="border-bottom: inset 1px #a5c3df">' +
                        '<span class="title">' +
                            '<span class="name"><i class="fa fa-user"></i><strong>  Kode : ' + escape(item.username) + '</strong></span>' +
                        '</span><br>' +
                        '<span class="description"><i class="fa fa-envelope"></i><strong>  Nmbu : ' + escape(item.email) + '</strong></span>' +
                        '</div>';
                    }
        },
        ...
        load: function(query, callback) {
            ...
            $.ajax({
                url: '{{url('admin/api/get-username')}}',
                type: 'POST',
                data: {
                    query: query,
                },
